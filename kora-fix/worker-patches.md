# ⚠ স্ট্যাটাস আপডেট (পুরো worker.js হাতে পাওয়ার পর)

আসল `worker.js` (৩৫৯৩ লাইন) পাওয়ার পর এই ফাইলের বেশ কিছু দাবি **ভুল প্রমাণিত হয়েছে**।
নিচের টেবিলটাই এখন সত্য; বিস্তারিত নিচের অনুচ্ছেদগুলোতে আগের লেখা ইতিহাস হিসেবে রয়ে গেছে।

| Patch | আগের দাবি | আসল অবস্থা |
|---|---|---|
| **A** | `autoBookPathao()` ফিক্স | ✅ **সম্পূর্ণ worker.js-এ বসানো হয়েছে** |
| **B** | নতুন হেল্পার | ✅ **বসানো হয়েছে** |
| **C** | "webhook 202 রিটার্ন করে না, secret echo করে না" | ❌ **দাবি ভুল ছিল — প্রত্যাহার**। লাইন 1213-এ আগেই `status: 202` + `X-Pathao-Merchant-Webhook-Integration-Secret` echo আছে। **তবে আসল একটা মারাত্মক বাগ পাওয়া গেছে:** ভেরিফিকেশন `X-Pathao-Signature` হেডার দিয়ে হতো — পাঠাও ওটা কখনো পাঠায় না। ফলে প্যানেলে webhook secret সেট করলে হ্যান্ডশেক ঠিকই হতো কিন্তু **প্রতিটা অর্ডার-ইভেন্ট 401 খেত**। এখন integration-secret হেডার দিয়ে যাচাই হয় → ✅ **ঠিক করা হয়েছে** |
| **D** | নতুন সেটিংস কী | ✅ **করা হয়েছে** — ৭টা কী seed + GET + POST + Courier প্যানেল, চার জায়গায় যুক্ত, আর `autoBookPathao` এখন সেগুলো **পড়ে ও ব্যবহার করে** |
| **E** | `local`/`call`/`online` চ্যানেল | ⏳ **বাকি** — `commerce_orders`-এ `CHECK (channel IN ('website','whatsapp'))` আছে (লাইন 109)। SQLite-এ CHECK বদলাতে টেবিল রিবিল্ড লাগে, তাই ঝুঁকি নিয়ে না করে আপনার সিদ্ধান্তের জন্য রাখা হয়েছে |
| **F** | "legacy `KR_SERVER_COUPONS` সরান" | ❌ **দাবি ভুল ছিল — প্রত্যাহার**। লাইন 2749 দেখাচ্ছে এটা মৃত কোড নয়, coupons টেবিলের migration চালানোর **আগের সেফটি নেট** (`if (!await couponsTableReady(env))`)। সরালে migration-এর আগে কুপন ভাঙত |
| **G** | নতুন `picked`/`manual` স্ট্যাটাস | ⏳ **বাকি** — schema + frontend দুটোতেই বদল লাগে |
