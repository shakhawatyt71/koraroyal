...
const KR_ADMIN = {
  ...
  NAV_ITEMS: [
    ...
    { id:'courier',   label:'Courier',    icon:'truck',        href:'courier.html'   },
...
const _SVG = {
  ...
  truck:    `<svg width="17" height="17" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><rect x="1" y="3" width="15" height="13"/><polygon points="16 8 20 8 23 11 23 16 16 16 16 8"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>`
  ,
};
window._SVG = _SVG;
...
/* ================================================================
   COURIER (Pathao) — শেয়ার্ড সেটিংস ও হেল্পার
   ----------------------------------------------------------------
   এখানে দুই ধরনের সেটিংস আছে:
     ① worker-backed  → /api/admin/operations-settings (সবার সাথে শেয়ার হয়)
     ② admin-local    → ব্রাউজারের localStorage (শুধু এই ব্রাউজারে)
   ② গুলো worker-এ এখনো কী নেই; worker patch হলে ①-তে চলে যাবে।
   orders.html আর courier.html দুটোই krCourierPrefs() দিয়েই পড়ে,
   তাই দুই জায়গায় আলাদা করে হার্ডকোড নেই।
   ================================================================ */
const KR_COURIER_PREFS_KEY = 'kr_courier_prefs_v1';

/* বাংলা instruction-এর শেষে আপনার love 𖹭 আইকন same to same রাখা হয়েছে।
   শুধু API booking-এ যায় (JSON UTF-8 — নিরাপদ)।
   CSV-তে যায় না, কারণ পাঠাওর bulk importer 4-byte অক্ষর ভেঙে দেয়। */
const KR_COURIER_DEFAULTS = {
  quantity: 1,
  amountSource: 'cod_remaining',
  roundAmount: true,
  secondaryPhone: false,
  descSku: true,
  descPrice: true,
  descQty: true,
  descFallback: 'Clothing',
  instrApi: 'আমাদের সম্মানিত গ্রাহক "{NAME}" স্যার/ম্যাম-এর পার্সেলটি খুব যত্নসহকারে পৌঁছে দিবেন। আর, উনি যদি পার্সেল রিটার্ন করেন, তাহলে {RETURN_CHARGE} টাকা ডেলিভারি চার্জ নেওয়ার চেষ্টা করবেন। ধন্যবাদ। 𖹭',
  instrCsv: 'Dear Pathao rider, please deliver this parcel of our valued customer "{NAME}" with care. If the customer returns the parcel, please try to collect {RETURN_CHARGE} BDT as delivery charge. Thank you.',
  autoCity: true,
  autoZone: true,
  autoArea: true,
  csvAscii: true,
...
function krCourierPrefs() {
  let saved = {};
  try { saved = JSON.parse(localStorage.getItem(KR_COURIER_PREFS_KEY) || '{}'); } catch (e) { saved = {}; }
  return Object.assign({}, KR_COURIER_DEFAULTS, saved);
}
function krSaveCourierPrefs(p) {
  try {
    localStorage.setItem(KR_COURIER_PREFS_KEY, JSON.stringify(p));
    return true;
  } catch (e) { return false; }
}

/* {NAME} {PHONE} {ORDER_ID} {RETURN_CHARGE} বসিয়ে দেয় */
function krApplyPlaceholders(tpl, ctx) {
  return String(tpl == null ? '' : tpl)
    .replace(/\{NAME\}/g, ctx.name || 'Valued Customer')
    .replace(/\{PHONE\}/g, ctx.phone || '')
    .replace(/\{ORDER_ID\}/g, ctx.orderId || '')
    .replace(/\{RETURN_CHARGE\}/g, String(ctx.returnCharge != null ? ctx.returnCharge : 130));
}

/* অর্ডারের পণ্য থেকে পাঠাওর "Item Description & Price" স্ট্রিং বানায়।
   opts.ascii = true হলে শুধু ASCII সেপারেটর ব্যবহার হয় (" - BDT "),
   নাহলে " — ৳"। CSV-তে ascii দরকার, কারণ পাঠাওর bulk importer
   নন-ASCII গাল্মিল করে দেয়; API booking-এ দরকার নেই (JSON UTF-8)। */
function krBuildItemDescription(items, prefs, opts) {
  const p = prefs || krCourierPrefs();
  const ascii = !!(opts && opts.ascii);
  const dash = ascii ? ' - ' : ' — ';
  const cur  = ascii ? 'BDT ' : '৳';
  const list = Array.isArray(items) ? items : [];
  const out = list.map(i => {
    let s = String(i.productName || i.name || '').trim();
    if (!s) return '';
    if (p.descSku && i.sku) s += ' [' + i.sku + ']';
    if (p.descQty) s += ' x' + (Number(i.quantity) || 1);
    if (p.descPrice && i.lineTotal != null) s += dash + cur + Math.round(Number(i.lineTotal));
    return s;
  }).filter(Boolean);
  return out.length ? out.join('; ') : (p.descFallback || 'Clothing');
}

/* Amount to Collect — prefs অনুযায়ী। codRemaining=0 হলেও 0 (প্রি-পেইড)। */
function krResolveCollectAmount(totals, prefs) {
  const t = totals || {}, p = prefs || krCourierPrefs();
  let v;
  if (p.amountSource === 'total_payable') {
    v = Number(t.totalPayable || 0);
  } else {
    v = (t.codRemaining != null) ? Number(t.codRemaining) : Number(t.totalPayable || 0);
  }
  if (!Number.isFinite(v)) v = 0;
  return p.roundAmount ? Math.round(v) : v;
}

/* নন-ASCII খুঁজে বের করে (CSV মজিবাইশ চেকের জন্য) */
function krNonAsciiChars(s) {
  const set = new Set();
  for (const ch of String(s == null ? '' : s)) if (ch.codePointAt(0) > 127) set.add(ch);
  return [...set];
}
